...
It is not well documented, what the Ender 3 S1 "eats", but it obviously handles unambiguous, complete, others call them "explicit" commands flawlessly. 
...
Lasergrbl however does not always supply explicit command lines, probably to,save bandwidth. If only the X-coordinate changes, then it supplies only an X-value, assuming that the laser cutter keeps the constant "sticky" commands and parameters.
...
I do not interfere with documentation of Lasergrbl, it is well documented. I put the setting to "Marlin" for generating gcode, but it did not seem to make much difference at first glance.
...
- Logging mode: A logfile is written with the input and output for each line processed for analysis later on.
None of the flags affects the result, just the speed of processing, which should not be an issue for normal PCs.
